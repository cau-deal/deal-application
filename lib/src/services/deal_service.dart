...
class DealService extends BaseService {
  ...
  DealService({@required this.client})
      : assert(client != null),
        super();

  static Future<DealService> init() async {
    DealServiceClient client =
        DealServiceClient(await GrpcClientSingleton.instance.channel, options: CallOptions(timeout: Duration(seconds: 5), metadata: {"ticket": "jwtjwt"}));
    return DealService(client: client);
  }

  Future<InquiryResponse> requestQuestion({String accessToken, String title, String content, InquiryCategory category}) async {
    ...
    try {
      ...
      res = await client.inquiry(req, options: CallOptions(metadata: {'ticket': accessToken}));
    } catch (e) {
      print(e.toString());
    }
  ...
  Future<LookUpInquiryResponse> fetchInquiryHistory({String accessToken}) async {
    ...
    try {
      res = await client.lookUpInquiry(req, options: CallOptions(metadata: {'ticket': accessToken}));
    } catch (e) {
      print(e.toString());
    }
